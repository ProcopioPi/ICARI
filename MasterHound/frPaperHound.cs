...
namespace MasterHound
{
    public partial class frPaperHound : Form
    {
        ...
        {
            Evaluate(1f, InfoManager.Instance.Info[infoIDx]);
        }

        private void BTN_DOWN_Click(object sender, EventArgs e)
        {
            Evaluate(.5f, InfoManager.Instance.Info[infoIDx]);
        }
        ...
        public void LoadArticleInfo(frMAIN parent, int idx, string projectName)
        {
            ...
            UpdateInfoInRichTxt(InfoManager.Instance.Info[infoIDx]);
            this.Show();
        }
        ...
        private void Evaluate(float evaluation, ArticleInfo info)
        {
            ...
            InfoManager.Instance.Info[infoIDx]  = info;
            FileManager.SaveAbstract(info);
            ...
            //parent.ChangeRowValues(frPaperHound.rowIndex, InfoManager.Instance.Info[idx]);
        ...
        {
            infoIDx++;
            infoIDx = (infoIDx % InfoManager.Instance.Info.Count);
            UpdateInfoInRichTxt(InfoManager.Instance.Info[infoIDx]);
        }
        ...
        {
            ArticleInfo tmp;
            tmp = InfoManager.Instance.Info.Find(x => x.ArticleStatus != ARTICLE_STATUS.EVALUATED);//AQUI ES DONDE QUEDA VACIO
            if (tmp != null)
            {
                return InfoManager.Instance.Info.FindIndex(x => x.id == tmp.id);
            }
            else
            {
                return -1;
